...
class LAD:
    ...
    def generate_chart(self):
        barsReq = StockBarsRequest(
            ...
            start=datetime.datetime(2024, 6, 24),
            end=datetime.datetime(2024, 6, 26),
            limit=self.limit
        )
        ...
        lad = talib.CDLLADDERBOTTOM(arrO, arrH, arrL, arrC)
        ...
        for i in points:
            fig.add_shape(
                type="circle",
                xref="x", yref="y",
                x0=time[i-4], x1=time[i],
                y0=min(arrL[i-5:i+1]) - 1, y1=max(arrH[i-5:i+1]) + 1,
        ...
        for i in range(len(mid)):
            if(i in points and (cash - amt * mid[i] > 10000) and (repeat or not buy)):
                ...
                print(f"We bought {amt} stocks for {mid[i]} on {i} because the {lad[i]} value")
                buy = True
