...
barsReq = StockBarsRequest(
    ...
    start=datetime.datetime(2024, 6, 28, 12),
    end=datetime.datetime(2024, 6, 28, 18),
    limit=50
)
...
eng = talib.CDLENGULFING(arrO, arrH, arrL, arrC)
...
for i in range(len(mid)):
    if(i in points and (cash - amt * mid[i] > 10000) and (repeat or not buy)):
        ...
        print(f"We bought {amt} stocks for {mid[i]} on {i} because the {eng[i]} value")
        buy = True
